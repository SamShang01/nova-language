...
# 查找函数对象
print("\n=== Looking for function objects in constants ===")
for i, const in enumerate(constants):
    print(f"Constant {i}: {type(const).__name__}")
    if hasattr(const, 'instructions'):
        print(f"  Function '{const.name}' instructions:")
        for j, func_instr in enumerate(const.instructions):
