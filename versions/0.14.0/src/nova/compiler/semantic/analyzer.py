...
class SemanticAnalyzer:
    ...
    def visit_ImplBlock(self, node):
        ...
        if node.trait_name:
            ...
            if isinstance(trait_type, TraitType):
                ...
                if type_name not in self.type_traits:
                    self.type_traits[type_name] = []
                if trait_type not in self.type_traits[type_name]:
                    self.type_traits[type_name].append(trait_type)
    ...
    def _check_method_signature(self, impl_method, trait_method):
        ...
        trait_params = getattr(trait_method, 'params', [])
