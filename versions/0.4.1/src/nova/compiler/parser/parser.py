...
class Parser:
    ...
    def parse_function_definition(self, is_generic=False):
        """
        解析函数定义
        ...
        return FunctionDefinition(
            token.line, token.column,
            name.lexeme, params, return_type, body
        )
    ...
    def parse_if_statement(self):
        ...
        if self.peek().type != TokenType.LBRACE:
            raise ParserError(self.peek().line, self.peek().column, "Expect '{' after condition")
        ...
        if self.peek().type == TokenType.ELSE:
            ...
            if self.peek().type == TokenType.IF:
                ...
                # 确保else后面跟着代码块
                if self.peek().type != TokenType.LBRACE:
                    raise ParserError(self.peek().line, self.peek().column, "Expect '{' after 'else'")
    ...
    def parse_struct_definition(self):
        ...
        """
        # 检查是否是泛型结构体
        is_generic = self.match(TokenType.GENERIC)
        ...
        if type_params is not None:
            return GenericStructDefinition(
                token.line, token.column,
                name.lexeme, type_params, fields, methods
            )
        
        return StructDefinition(token.line, token.column, name.lexeme, fields, methods)
    ...
    def parse_enum_definition(self):
        ...
        return EnumDefinition(token.line, token.column, name.lexeme, variants)
    ...
    def parse_trait_definition(self):
        ...
        return TraitDefinition(token.line, token.column, name.lexeme, methods)
    ...
    def parse_impl_block(self):
        ...
        type_name = self.consume(TokenType.IDENTIFIER, "Expect type name")
        
        # 可选的for关键字
        trait_name = None
        if self.peek().type == TokenType.FOR:
            self.advance()
            trait_name = type_name.lexeme
            type_name = self.consume(TokenType.IDENTIFIER, "Expect type name after 'for'")
        ...
        return ImplBlock(token.line, token.column, type_name.lexeme, methods, trait_name)
    ...
    def parse_comparison(self):
        ...
        while True:
            ...
            if self.match(
                ...
                # 特殊处理：如果是 < 操作符，且左侧是标识符，可能是泛型类型实例化
                if self.previous().type == TokenType.LESS_THAN:
                    # 检查左侧是否是标识符
                    if isinstance(expression, IdentifierExpression):
                        # 检查下一个token是否是标识符（类型名）
    ...
    def parse_primary(self):
        ...
        if self.match(TokenType.FALSE):
            ...
            # 检查是否是调用
            if self.peek().type == TokenType.LPAREN:
                return self.parse_call(IdentifierExpression(
    ...
    def consume(self, type, message):
        ...
        if self.check(type):
            return self.advance()
        raise ParserError(self.peek().line, self.peek().column, message)
    ...
    def parse_where_clause(self):
        ...
        while True:
            ...
            trait_name = self.consume(TokenType.IDENTIFIER, "Expect trait name")
            
            constraints.append(TypeConstraint(
                type_param.line, type_param.column,
                type_param.lexeme, trait_name.lexeme
            ))
    ...
    def parse_type(self):
        ...
        if self.peek().type == TokenType.FUNC:
            ...
            self.consume(TokenType.ARROW, "Expect '->' after parameter types")
            return_type = self.parse_type()
