...
def reversed_func(iterable):
    ...
    Returns:
        list: 反转后的列表
    """
    return builtins.list(builtins.reversed(iterable))
...
def enumerate_func(iterable, start=0):
    ...
    Returns:
        list: 枚举后的列表
    """
    return builtins.list(builtins.enumerate(iterable, start))

def range_func(start, end=None, step=1):
    """
    范围函数
...
def isinstance_func(value, type_name):
    """
    类型检查函数
...
def contains_func(container, item):
    """
    包含检查函数
...
def sort_func(lst, reverse=False):
    """
    列表排序函数
...
def slice_func(lst, start=None, end=None, step=None):
    """
    切片函数
