...
class SemanticAnalyzer:
    ...
    def _initialize_builtins(self):
        ...
        # 初始化内置Trait
        self._initialize_builtin_traits()
        
        # 初始化Vector类型
        self._initialize_vector_type()
    
    def _initialize_builtin_traits(self):
        """
        初始化内置Trait
        """
        # Comparable trait
        comparable_trait = TraitType("Comparable", methods=[
            ("compare", FunctionType([ANY_TYPE], INT_TYPE)),
            ("__lt__", FunctionType([ANY_TYPE], BOOL_TYPE)),
            ("__le__", FunctionType([ANY_TYPE], BOOL_TYPE)),
            ("__gt__", FunctionType([ANY_TYPE], BOOL_TYPE)),
            ("__ge__", FunctionType([ANY_TYPE], BOOL_TYPE)),
        ])
        self.types["Comparable"] = comparable_trait
        
        # Display trait
        display_trait = TraitType("Display", methods=[
            ("toString", FunctionType([], STRING_TYPE)),
        ])
        self.types["Display"] = display_trait
        
        # Iterable trait
        iterable_trait = TraitType("Iterable", methods=[
            ("iterator", FunctionType([], ANY_TYPE)),
        ])
        self.types["Iterable"] = iterable_trait
        
        # Addable trait
        addable_trait = TraitType("Addable", methods=[
            ("__add__", FunctionType([ANY_TYPE], ANY_TYPE)),
        ])
        self.types["Addable"] = addable_trait
    ...
    def visit_FunctionDefinition(self, node):
        ...
        if not node.return_type:
            inferred_return_type = self._infer_return_type(node.body, func_scope)
    ...
    def visit_GenericFunctionDefinition(self, node):
        ...
        # 在全局作用域中声明函数（允许覆盖已有符号）
        try:
            self.current_scope.declare_symbol(func_symbol)
        except ValueError:
            # 如果符号已存在，覆盖它
            self.current_scope.symbols[func_symbol.name] = func_symbol
    ...
    def visit_ReturnStatement(self, node):
        ...
        if node.value:
            ...
            # 如果函数返回类型是UNIT_TYPE（未指定），则更新为实际返回类型
            if func_symbol.return_type == UNIT_TYPE:
                func_symbol.return_type = return_type
            elif not self._types_compatible(return_type, func_symbol.return_type):
                raise SemanticError(
                    node.line, node.column,
    ...
    def visit_BinaryExpression(self, node):
        ...
        if node.operator == "=":
            ...
            # 对于其他二元操作，检查类型兼容性（双向检查，允许int和float混合）
            if not (self._types_compatible(left_type, right_type) or self._types_compatible(right_type, left_type)):
                raise SemanticError(
                    node.line, node.column,
            ...
            # 对于算术操作，如果一个是float，返回float
            if node.operator in ("+", "-", "*", "/", "%"):
                if (isinstance(left_type, PrimitiveType) and left_type.name == "float") or \
                   (isinstance(right_type, PrimitiveType) and right_type.name == "float"):
                    return FLOAT_TYPE
    ...
    def visit_LambdaExpression(self, node):
        ...
        # 分析函数体
        # node.body 可能是单个表达式/语句，也可能是语句列表
        body_type = UNIT_TYPE
        if isinstance(node.body, list):
            ...
            if node.body:
                ...
                if hasattr(last_stmt, 'value') and last_stmt.value is not None:
                    body_type = self._infer_type(last_stmt.value)
                elif hasattr(last_stmt, 'expression') and last_stmt.expression is not None:
                    body_type = self._infer_type(last_stmt.expression)
        else:
            # 单个表达式/语句
    ...
    def _infer_return_type(self, body, parent_scope=None):
        """
        推断函数返回类型
        
        Args:
            body: 函数体语句列表
            parent_scope: 父作用域（用于访问函数参数）
        
        Returns:
            Type: 推断的返回类型，如果没有return语句则返回None
        """
        # 临时作用域，用于记录变量声明
        temp_scope = Scope("temp", parent=parent_scope if parent_scope else self.current_scope)
        old_scope = self.current_scope
        self.current_scope = temp_scope
