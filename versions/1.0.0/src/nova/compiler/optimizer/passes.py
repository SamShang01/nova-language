...
from nova.compiler.parser.ast import BinaryExpression, LiteralExpression, FunctionDefinition, ReturnStatement, CallExpression, IdentifierExpression, IfStatement, LoopStatement
...
class TailRecursionOptimization(OptimizationPass):
    """
    尾递归优化
    ...
    def optimize(self, ast):
        ...
        Returns:
            Node: 优化后的AST
        """
        return self._optimize_tail_recursion(ast)
    
    def _optimize_tail_recursion(self, node):
        """
        递归优化尾递归
        ...
        if isinstance(node, FunctionDefinition):
            ...
            if self._is_tail_recursive(node):
                # 转换为迭代版本
                return self._convert_to_iteration(node)
        
        # 递归处理所有子节点
        for attr_name in dir(node):
            if not attr_name.startswith('_'):
                attr_value = getattr(node, attr_name, None)
                if isinstance(attr_value, FunctionDefinition):
                    setattr(node, attr_name, self._optimize_tail_recursion(attr_value))
                elif isinstance(attr_value, list):
                    setattr(node, attr_name, [self._optimize_tail_recursion(item) if isinstance(item, FunctionDefinition) else item for item in attr_value])
    ...
    def _is_tail_recursive(self, func_def):
        ...
        # 检查函数体中是否有递归调用
        return self._has_tail_call(func_def.body, func_def.name)
    
    def _has_tail_call(self, node, func_name):
        """
        检查节点中是否有尾递归调用
        ...
        if isinstance(node, ReturnStatement):
            # 检查返回的表达式是否是函数调用
            if hasattr(node, 'value') and isinstance(node.value, CallExpression):
                # 检查是否是递归调用
                if hasattr(node.value.callee, 'name') and node.value.callee.name == func_name:
                    return True
        ...
        for attr_name in dir(node):
            if not attr_name.startswith('_'):
                attr_value = getattr(node, attr_name, None)
                if isinstance(attr_value, ReturnStatement):
                    if self._has_tail_call(attr_value, func_name):
                        return True
                elif isinstance(attr_value, list):
                    for item in attr_value:
                        if isinstance(item, ReturnStatement) and self._has_tail_call(item, func_name):
                            return True
        
        return False
    
    def _convert_to_iteration(self, func_def):
        """
        将尾递归函数转换为迭代版本
