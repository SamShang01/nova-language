...
class Parser:
    ...
    def parse_impl_block(self):
        ...
        if trait_name is not None:
            ...
            type_name_str = trait_name.name if hasattr(trait_name, 'name') else str(trait_name)
            trait_name_expr = type_name_expr  # 保持 trait_name 作为 TypeExpression 对象
        else:
            # 没有 for 关键字：impl Type
            type_name_str = type_name_expr.name if hasattr(type_name_expr, 'name') else str(type_name_expr)
            trait_name_expr = None
        
        return ImplBlock(token.line, token.column, type_name_str, methods, trait_name_expr, type_params)
