...
class CodeGenerator:
    ...
    def visit_ImplBlock(self, node):
        ...
        if hasattr(node.type_name, 'name'):
            ...
            # 泛型类型表达式，尝试提取类型名称
            if hasattr(node.type_name, 'type_name'):
                # GenericTypeExpression
                type_name = node.type_name.type_name
            else:
                # 无法识别的类型，暂时跳过
                return
        ...
        if nova_class is None:
            for instr in reversed(self.instructions):
                if hasattr(instr, 'name') and instr.name == 'STORE_NAME':
                    if hasattr(instr, 'value') and isinstance(instr.value, NovaClass):
                        if instr.value.name == type_name:
                            nova_class = instr.value
                            break
        ...
        for method in node.methods:
            if hasattr(method, 'name'):
                ...
                # 将方法添加到类中
                nova_class.methods.append((method.name, method_func))
