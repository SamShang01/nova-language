...
class NovaREPL:
    ...
    def _execute_code(self, code: str):
        ...
        try:
            ...
            # 保存原始作用域
            original_scope = self.analyzer.current_scope
            ...
            # 从语义分析器的作用域中获取变量的声明类型
            self._sync_variable_types()
    ...
    def _sync_environment(self):
        ...
        for name, value in self.vm.environment.items():
            # 跳过内置函数和内置类型
            if name in ['print', 'len', 'input'] or name in self.analyzer.types:
                continue
    ...
    def _sync_variable_types(self):
        """
        同步变量的声明类型到REPL
        """
        # 从全局作用域中获取变量类型
        for name, symbol in self.analyzer.global_scope.symbols.items():
            if hasattr(symbol, 'type') and not callable(self.vm.environment.get(name, None)):
                # 保存变量的声明类型
                self.variable_types[name] = str(symbol.type)
    ...
    def _show_type_info(self, name: str):
        ...
        elif name in self.analyzer.types:
            # 显示类型信息
            type_obj = self.analyzer.types[name]
            print(f"{name}: type")
