    ...
    if hasattr(stmt, 'name') and stmt.name == 'apply':
        for param in stmt.params:
            if hasattr(param, 'name') and param.name == 'transformer':
                print(f"Testing _resolve_type for {param.name}")
                print(f"param_type: {param.param_type}")
                print(f"has param_types: {hasattr(param.param_type, 'param_types')}")
